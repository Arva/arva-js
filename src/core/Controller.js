...
export class Controller extends EventEmitter {
     ...
     * @returns {Promise<Boolean>} success Whether the controller method was fully executed, and the Router should emit a routechange event.
     */
    async onRouteCalled(route) {
        this.isActive = true;
        if (typeof this[route.method] !== 'function') {
            console.log('Route does not exist!');
            return false;
        }
        let result = await this[route.method].apply(this, route.values);
    ...
    _showView(view, route) {
        ...
        if (view instanceof Dialog) {
            if (dialogManager.getOpenDialog() !== view) {
                dialogManager.show({
                    dialog: view,
                    canCancel: false,
                    shouldGoToRoute: view.goToRoute || this.router.getPreviousRoute()
                });
                dialogManager.once('dialogShown', () => {
